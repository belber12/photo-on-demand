...
const TooltipContent = React.forwardRef(({ className, sideOffset = 4, ...props }, ref) => (
  <TooltipPrimitive.Content
    ...
    className={cn(
      "z-50 overflow-hidden rounded-md border border-[#333] bg-[#1F2023] px-3 py-1.5 text-sm text-white shadow-md animate-in fade-in-0 zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95",
      className
    )}
    {...props}
  />
    ...
    return (
      <TooltipProvider>
        <PromptInputContext.Provider value={{ isLoading, value: value ?? internalValue, setValue: onValueChange ?? handleChange, maxHeight, onSubmit, disabled }}>
          <div
            ref={ref}
            className={cn("rounded-3xl border border-[#444] bg-[#1F2023] p-2 shadow-[0_8px_30px_rgba(0,0,0,0.24)] transition-all duration-300", isLoading && "border-red-500/70", className)}
...
export const PromptInputBox = React.forwardRef((props, ref) => {
  ...
  const handleToggleChange = (val) => {
    if (val === "search") { setShowSearch((p) => !p); setShowThink(false); }
    else if (val === "think") { setShowThink((p) => !p); setShowSearch(false); }
  };
  ...
  return (
    <>
      <PromptInput
        ...
        className={cn("w-full bg-[#1F2023] border-[#444] shadow-[0_8px_30px_rgba(0,0,0,0.24)] transition-all duration-300", isRecording && "border-red-500/70", className)}
      ...
      >
              ...
              file.type.startsWith("image/") && filePreviews[file.name] && (
                <div key={i} className="relative group">
                  <div className="w-16 h-16 rounded-xl overflow-hidden cursor-pointer" onClick={() => setSelectedImage(filePreviews[file.name])}>
                    ...
                    <button onClick={(e) => { e.stopPropagation(); handleRemoveFile(i); }} className="absolute top-1 right-1 rounded-full bg-black/70 p-0.5">
        ...
        <div className={cn("transition-all duration-300", isRecording ? "h-0 overflow-hidden opacity-0" : "opacity-100")}>
          <PromptInputTextarea
            placeholder={showSearch ? "Поиск в интернете..." : showThink ? "Глубокий анализ..." : showCanvas ? "Создать на холсте..." : placeholder}
            className="text-base"
          />
        </div>
        ...
        {isRecording && (
          <VoiceRecorder
            ...
            onStartRecording={() => {}}
            onStopRecording={(dur) => { setIsRecording(false); onSend(`[Голосовое сообщение — ${dur} сек]`, []); }}
          />
        )}
        ...
        <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
          ...
          {/* Send / Record button */}
          <PromptInputAction tooltip={isLoading ? "Остановить" : isRecording ? "Стоп запись" : hasContent ? "Отправить" : "Голосовое"}>
            <Button
              variant="default"
              size="icon"
              className={cn(
